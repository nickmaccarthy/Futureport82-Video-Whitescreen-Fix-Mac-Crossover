...
# Copy a single DLL: try without sudo first, escalate only if needed.
# The original -w check fails for non-existent files (returns false),
# incorrectly routing all new DLL copies through sudo. This version
# tries the operation directly and only uses sudo on failure.
function copy_dll {
	local src="$1"
	local dest_dir="$2"
	local label="$3"
	...
	if [[ -e "${dest}" ]] || [[ -h "${dest}" ]]; then
		if rm -f "${dest}" 2>/dev/null; then
			echo "${label}/$(basename "${src}"): removed existing"
		elif gui_sudo rm -f "${dest}" 2>&1; then
			echo "${label}/$(basename "${src}"): removed existing (admin)"
		else
			echo "Warning: Cannot remove $(basename "${src}"), skipping"
			return 1
		fi
	fi
	...
	if cp "${src}" "${dest}" 2>/dev/null; then
		echo "${label}/$(basename "${src}"): copied"
	elif gui_sudo cp "${src}" "${dest}" 2>&1; then
		echo "${label}/$(basename "${src}"): copied (admin)"
	else
		echo "Warning: Cannot copy $(basename "${src}")"
		return 1
	fi
}
...
function run_copies {
	cd "${mydir}/system32"
	for f in *; do
		copy_dll "${f}" "${WPATH}/drive_c/windows/system32" "system32" || true
	done

	cd "${mydir}/syswow64"
	for f in *; do
		copy_dll "${f}" "${WPATH}/drive_c/windows/syswow64" "syswow64" || true
	done
}
